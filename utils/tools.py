...
def to_device(data, device,speaker_emb=False, non_blocking=True):
    if len(data) == 12:
        (
            ids,
            raw_texts,
        ...
        if not speaker_emb:
            speakers = torch.from_numpy(speakers).long().to(device, non_blocking=non_blocking)
        ...
        energies = torch.from_numpy(energies).to(device, non_blocking=non_blocking)
